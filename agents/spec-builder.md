...
4. **Author the SPEC** following the section order in `skills/spec-format`. Required sections: Header, Problem Statement, Acceptance Criteria, Target Repo, Files to Touch, Ordered Implementation Steps, Risks & Assumptions, Verification Plan, Out of Scope, Git Strategy, Traceability. **Structure** is optional — include only when work splits into internal slices (see `skills/spec-format § Structure` for when it applies).
