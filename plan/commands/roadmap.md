---
description: Decompose an approved PRD into a phased, ordered roadmap with shippable milestones
...
Decompose an approved PRD into a phased, ordered roadmap with shippable milestones. Each milestone maps to one or more `/plan:sdd` specs.
...
- If score < 70: **STOP** — display findings and tell the user: "PRD needs work before roadmap generation. Run `/plan:validate {prd_path}` to see findings, then update the PRD and re-run `/plan:roadmap`."
...
For each milestone's checklist, include a `/plan:sdd` invocation hint:
```
- [ ] Spec: `/plan:sdd [milestone description]` → `docs/{feature}/specs/SPEC-NNN-{slug}.md`
```
...
Suggest next step: "Roadmap saved. Run `/plan:sdd [milestone description]` to spec each milestone as `docs/{feature}/specs/SPEC-NNN-{slug}.md`."
