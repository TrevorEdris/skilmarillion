...
Decompose an approved PRD into a phased, ordered roadmap with shippable milestones. Each milestone maps to a spec generated by `/plan:specify`.
...
- **Estimated scope** — TRIVIAL, SMALL, or FEATURE (informs spec depth when `/plan:specify` generates the spec — see `spec-format` skill for section gating)
...
Suggest next step: "Roadmap saved. Run `/plan:specify {roadmap-path}` to generate all specs."
...
> **Roadmap saved.** Next step: run `/plan:specify` to generate all specs from this roadmap.
>
> ```
