...
>
> **Next steps:**
> - To design the API: `/fellowship:plan --arch api [api-name]`
> - To design the database schema: `/fellowship:plan --arch schema [name]`
> - To generate additional diagrams: `/fellowship:plan --arch diagram [description]`
> - To create a spec for implementation: `/fellowship:plan [task]`

If the `plan` plugin is not installed, append:

> The `plan` plugin is not installed. Install with: `/plugin install fellowship@skilmarillion` (already present)
...
> The `impl` plugin is not installed. Install with: `/plugin install fellowship@skilmarillion` (already present)
...
> {count} specs found. Most recent: `{spec-path}` -- `/fellowship:build {spec-path}` will pick up both the spec and these design artifacts.
