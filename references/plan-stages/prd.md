...
8. **Suggest next step:** "PRD saved. Run `/fellowship:plan --roadmap {prd-path}` to decompose into milestones, then `/fellowship:plan --specify {roadmap-path}` to generate all specs."
...
> **PRD saved.** Next step:
> ```
> /fellowship:plan --roadmap {prd-path}
> ```
