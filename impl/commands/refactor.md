---
description: Phase-gated refactoring with test verification between every transformation step. Never adds behavior during refactor.
...
3. **If red:** STOP. Do not refactor against a failing suite. Report the failures and suggest `/impl:debug` instead.
...
5. Suggest: "Run `/review:review` to validate the refactoring" (with install hint if `review` plugin is not present).
...
- **3 consecutive RED results on the same step:** Stop attempting. Report the pattern and suggest `/impl:debug` for root cause analysis.
